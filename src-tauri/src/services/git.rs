...
impl GitService {
    ...
    pub fn blame_file(
        path: &Path,
        file_path: &str,
    ) -> Result<GitBlameResult, GitServiceError> {
        let repo = Self::open_repository(path)?;
        ...
        for line_content in content.lines() {
            ...
            let original_line_number = blame_hunk.as_ref()
                .map(|h| h.final_start_line())
                .unwrap_or(line_number);

            let (commit_sha, short_sha, author, author_email, timestamp, summary) = if let Some(hunk) = blame_hunk {
                ...
                ("unknown".to_string(), "unknown".to_string(), "Unknown".to_string(), "".to_string(), 0, None)
            };
